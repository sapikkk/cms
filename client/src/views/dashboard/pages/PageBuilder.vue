<template>
  <div class="h-screen flex flex-col md:flex-row bg-cream-100 dark:bg-green-950 transition-colors duration-300">
    <!-- Left Panel: Available Widgets -->
    <aside class="w-full md:w-64 bg-white dark:bg-green-900 border-r border-green-200 dark:border-green-800 p-4 overflow-y-auto flex-shrink-0 z-10 shadow-sm md:shadow-none">
      <div class="font-bold text-green-800 dark:text-white mb-4 flex items-center gap-2">
      ...
      <div class="space-y-2">
        ...
        >
          <div class="p-2 bg-white dark:bg-green-800 rounded-md shadow-sm group-hover:shadow text-brand-orange">
             <component :is="widget.iconComponent" :size="20" />
          </div>
          <span class="text-sm font-bold text-green-700 dark:text-white">{{ widget.label }}</span>
        </div>
      </div>
      ...
      <div class="mt-8 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
        <p class="text-xs text-blue-800 dark:text-blue-300 font-medium">
          💡 Tips: Klik widget untuk menambahkannya ke canvas. Drag & drop untuk mengatur posisi.
        </p>
      </div>
    </aside>

    <!-- Center: Editor Area (Tabs + Form) -->
    <main class="flex-1 bg-cream-200 dark:bg-green-950 flex flex-col min-w-0 overflow-hidden">
      <!-- Toolbar -->
      <div class="px-6 py-4 bg-white dark:bg-green-900 border-b border-green-200 dark:border-green-800 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-xl font-bold text-green-800 dark:text-white flex items-center gap-2">
              {{ pageTitle }}
              <PhPencilSimple :size="16" class="text-green-400 cursor-pointer hover:text-brand-orange" />
            </h1>
            <div class="flex items-center gap-2 text-xs">
              <span v-if="saving" class="text-brand-orange flex items-center gap-1">
        ...
        <div class="flex items-center gap-2">
          <button
            ...
            :class="{ 'text-brand-orange bg-orange-50 dark:bg-green-800': showDebug }"
      ...
      <!-- Tabs Navigation -->
      <div class="px-6 pt-4 bg-white dark:bg-green-900 border-b border-green-200 dark:border-green-800 overflow-x-auto shrink-0">
        <nav class="flex gap-1 min-w-max pb-px">
           ...
           >
             ...
             Atur Urutan
             <span class="bg-green-100 dark:bg-green-800 text-green-700 dark:text-green-300 text-xs px-1.5 py-0.5 rounded-full ml-1">{{ sections.length }}</span>
           </button>
        </nav>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-6">
        <div class="max-w-4xl mx-auto">
          ...
          <!-- Empty State -->
          <div v-if="sections.length === 0" class="flex flex-col items-center justify-center py-20 bg-white dark:bg-green-900/40 rounded-xl border border-dashed border-green-300 dark:border-green-700">
            <div class="p-4 bg-cream-100 dark:bg-green-800 rounded-full mb-4 text-brand-orange">
              <PhPackage :size="48" weight="duotone" />
            </div>
            <h3 class="text-xl font-bold text-green-800 dark:text-white mb-2">Halaman Kosong</h3>
            <p class="text-green-600 dark:text-gray-400 text-center max-w-sm mb-6">
              Belum ada konten di halaman ini. Pilih widget dari panel kiri untuk menambahkan konten.
            </p>
          </div>

          <!-- Section Editor Form -->
          <div v-if="activeSection" class="bg-white dark:bg-green-900 border border-green-200 dark:border-green-800 rounded-xl shadow-sm overflow-hidden">
             <div class="p-6 border-b border-green-100 dark:border-green-800 flex justify-between items-start bg-green-50/50 dark:bg-green-800/30">
               <div>
                  <h2 class="text-lg font-bold text-green-800 dark:text-white flex items-center gap-2">
                    <component :is="getWidgetIcon(activeSection.type)" :size="24" class="text-brand-orange" />
                    Edit {{ getWidgetLabel(activeSection.type) }}
                  </h2>
                  <p class="text-sm text-green-600 dark:text-green-400 mt-1">Sesuaikan konten dan tampilan section ini.</p>
               </div>
               <button 
                 @click="removeSection(sections.findIndex(s => s.id === activeTab))"
             ...
             <div class="p-6 space-y-6">
                ...
                <div v-for="(value, key) in activeSection.props" :key="key">
                  <label class="block text-sm font-bold text-green-700 dark:text-gray-300 mb-2 capitalize">
                    {{ formatLabel(key) }}
                  </label>
                  
                  <!-- Array Editor (For features, stats, items, etc.) -->
                  <div v-if="Array.isArray(value)" class="space-y-3">
                    <div v-for="(item, index) in activeSection.props[key]" :key="index" 
                      ...
                      <div v-if="typeof item === 'object' && !Array.isArray(item)" class="space-y-2 pr-8">
                        <div v-for="(propValue, propKey) in item" :key="propKey">
                          <label class="block text-xs font-medium text-green-600 dark:text-green-400 mb-1">{{ propKey }}</label>
                          <input 
                  ...
                  <!-- Rich Text / Long Text -->
                  <div v-else-if="key === 'content' || key === 'subtitle' || key === 'description' || key === 'title'" class="relative">
                     <RichTextEditor
                  ...
                  <!-- Image URL Special Input (If key contains 'image' or 'url') -->
                  <div v-else-if="typeof value === 'string' && (key.toLowerCase().includes('image') || key.toLowerCase().includes('url'))" class="space-y-3 p-4 border border-green-200 dark:border-green-700 rounded-xl bg-white dark:bg-green-900/50">
                    <div class="flex items-center gap-3">
                      <div class="w-16 h-16 rounded-lg bg-gray-100 dark:bg-green-800 border border-green-200 dark:border-green-700 overflow-hidden flex items-center justify-center shrink-0 shadow-sm relative group">
                        <img v-if="activeSection.props[key]" :src="activeSection.props[key]" class="w-full h-full object-cover" />
                        <PhImage v-else :size="24" class="text-gray-400" />
                      ...
                      <div class="flex-1 space-y-2">
                        ...
                        />
                        <p class="text-[10px] text-green-500 dark:text-gray-400">
                          Paste link gambar langsung (Google Drive, Unsplash, hosted URL).
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div v-else-if="typeof value === 'object' && !Array.isArray(value)" class="relative">
                    <textarea
                    ...
                    ></textarea>
                    <span class="text-xs text-green-500 dark:text-green-400">Klik di luar field untuk menerapkan perubahan JSON</span>
                  </div>
          ...
          <div v-else-if="activeTab === 'order'" class="bg-white dark:bg-green-900 border border-green-200 dark:border-green-800 rounded-xl shadow-sm p-6">
             <h2 class="text-lg font-bold text-green-800 dark:text-white mb-4 flex items-center gap-2">
               <PhListNumbers :size="24" class="text-brand-orange" />
               Urutan Section
             </h2>
             <p class="text-sm text-green-600 dark:text-green-400 mb-6">
               Drag dan drop item di bawah ini untuk mengatur urutan tampilan section di halaman publik.
             </p>
             ...
             >
                <template #item="{ element: section, index }">
                  <div class="flex items-center gap-4 p-4 border border-green-200 dark:border-green-800 rounded-lg bg-cream-50 dark:bg-green-900/50 hover:bg-white dark:hover:bg-green-800 transition-colors group">
                    <button class="drag-handle p-2 text-green-400 hover:text-green-600 cursor-grab active:cursor-grabbing">
                      <PhList :size="20" />
                    </button>
                    <div class="p-2 bg-white dark:bg-green-800 rounded border border-green-100 dark:border-green-700 text-brand-orange">
                    ...
                    <div class="flex-1">
                      ...
                      <div class="text-xs text-green-500 font-mono mt-0.5">ID: {{ section.id }}</div>
                    </div>
                    <div class="text-xs font-bold bg-green-200 dark:bg-green-700 text-green-800 dark:text-green-100 px-2.5 py-1 rounded-full">
                       Posisi {{ index + 1 }}
                    </div>
                  </div>
                </template>
    ...
    <div v-if="showDebug" class="fixed bottom-0 left-0 right-0 bg-black/90 text-green-400 p-4 font-mono text-xs z-[9999] h-48 overflow-auto border-t-2 border-green-500">
...
import { 
  PhPackage, PhArrowLeft, PhPencilSimple, PhEye, PhFloppyDisk, 
  PhArrowUp, PhArrowDown, PhGear, PhTrash, PhLayout, PhFaders, PhX,
  PhImage, PhSquaresFour, PhTextT, PhTextbox,
  PhCoffee, PhLightning, PhBook, PhMusicNotes, PhWifiHigh,
  PhBookOpen, PhPaintBrush, PhStar, PhUser, PhCamera,
  PhCheckCircle, PhQuestion, PhUserCircle, PhEnvelopeSimple,
  PhPaperPlaneTilt, PhBuildings, PhPlayCircle, PhMapPin, PhPhone,
  PhArrowsOutLineHorizontal, PhBug, PhList, PhListNumbers, PhSpinner,
  PhPlusCircle, PhBell
...
const NotificationSection = defineAsyncComponent(() => import('@/views/storefront/sections/NotificationSection.vue'))
...
const availableWidgets = [
  { type: 'HeroBannerCenter', label: 'Hero - Center', iconComponent: PhImage, defaultProps: { 
    badgeText: 'Marpoyan, Pekanbaru',
    title: 'Bukan Cuma<br><span class="text-orange-500">Tempat Ngopi</span>',
    subtitle: 'Library. Workspace. Coffee. Tiga elemen yang bikin lu produktif sambil tetep santai. <b>Antitesa</b> bukan tempat nongkrong biasa.',
  ...
  { type: 'HeroSplitRight', label: 'Hero - Image Right', iconComponent: PhImage, defaultProps: { 
    badge: 'Premium Experience',
    headingLine1: 'Kopi Bukan',
    headingLine2Part1: 'Sekadar',
    headingLine2Part2: 'Minuman',
    description: 'Ini tentang ritual. Tentang menemukan momen di tengah kesibukan. Tentang menyeduh pikiran sambil menikmati seduhan terbaik.',
    features: [
    ...
    imageUrl: '',
    quoteText: 'Kopi yang baik datang dari biji yang baik. Pikiran yang jernih datang dari ruang yang tenang.',
    quoteAuthor: 'Filosofi Antitesa'
  } },
  { type: 'HeroSplitLeft', label: 'Hero - Image Left', iconComponent: PhImage, defaultProps: { 
    badge: 'DI JALAN MARPOYAN',
    headingLine1: 'Ruang Baca.',
    headingLine2: 'Ruang Kerja.',
    headingLine3: 'Ruang Pikir.',
    description: 'Gak cuma sekedar wifi gratisan dan kursi empuk. Di sini lu dapet akses ke ribuan buku, meja kerja yang proper, sama kopi yang bikin otak fresh.',
    features: [
  ...
  { type: 'ProductGrid', label: 'Product Grid (API)', iconComponent: PhSquaresFour, defaultProps: { 
    title: 'Koleksi <span class="text-orange-500">Terbaik</span> Kami',
    subtitle: 'Dipilih khusus untuk menemani waktu produktif anda',
    limit: 8,
    showLink: true
  } },
  { type: 'NotificationSection', label: 'Announcements', iconComponent: PhBell, defaultProps: { 
    title: 'Pengumuman Terbaru', 
    limit: 5 
  } },
  { type: 'FeatureGrid3', label: 'Features (3 Col)', iconComponent: PhSquaresFour, defaultProps: { 
    title: 'Tiga Pilar <span class="text-orange-500">Antitesa</span>',
    subtitle: 'Bukan cuma tempat ngopi. Ini ekosistem lengkap buat lu yang pengen produktif sambil tetep nyaman.',
    features: [
      { title: 'Premium Coffee', description: 'Single origin beans, manual brew methods, barista bersertifikat.', items: [{ text: 'Biji kopi dari petani lokal pilihan' }, { text: 'V60, Chemex, French Press' }] },
      { title: 'Curated Library', description: '500+ buku pilihan dari filosofi hingga bisnis.', items: [{ text: 'Koleksi klasik & kontemporer' }, { text: 'Reading corner nyaman' }] },
      { title: 'Pro Workspace', description: '24 meja kerja dengan WiFi 100 Mbps.', items: [{ text: 'Private & shared workspace' }, { text: 'Silent zone' }] }
    ],
    bottomText: 'Tiga elemen ini bukan cuma <em>exist</em>. Mereka <strong>synergize</strong>.',
    buttonText: 'Experience the Difference →'
  } },
  { type: 'FeatureGrid4', label: 'Features (4 Col)', iconComponent: PhSquaresFour, defaultProps: { title: 'What We Offer' } },
  { type: 'ContentImageRight', label: 'Content + Image Right', iconComponent: PhTextbox, defaultProps: { 
    badge: 'Philosophy',
    title: '<span class="text-orange-500">Slow Down,</span> <br/>Think Deep',
    description: 'Di dunia yang serba cepat, kita tawarkan sesuatu yang berbeda: ruang untuk melambat, berpikir, dan berkembang.',
    content: 'Setiap detail di Antitesa dirancang untuk mendukung deep work dan meaningful conversation.',
    items: [{ text: 'Noise level terkontrol' }, { text: 'Ergonomic furniture' }, { text: 'Natural lighting balance' }],
    buttonText: 'Experience It Yourself',
    imageUrl: ''
  } },
  { type: 'ContentImageLeft', label: 'Content + Image Left', iconComponent: PhTextbox, defaultProps: { 
    badge: 'Our Story',
    title: 'Lahir dari Kecintaan pada <span class="text-orange-500">Kopi & Buku</span>',
    description: 'Antitesa dimulai dari pertanyaan sederhana: kenapa gak ada tempat yang combine kopi berkualitas dengan library yang proper?',
    content: 'Kita percaya ruang mempengaruhi pikiran. Pikiran mempengaruhi karya. Dan karya mengubah dunia.',
    buttonText: 'Baca Selengkapnya →',
    imageUrl: ''
  } },
  { type: 'TextBlock', label: 'Text Block', iconComponent: PhTextT, defaultProps: { title: 'About Us', content: 'Share your story here...' } },
  { type: 'Stats3Col', label: 'Stats (3 Columns)', iconComponent: PhSquaresFour, defaultProps: { title: 'Numbers Speak' } },
  { type: 'CTACentered', label: 'CTA - Centered', iconComponent: PhPackage, defaultProps: { 
    title: 'Siap Upgrade <br/><span class="text-orange-500">Pengalaman Ngopi</span> Lu?',
    description: 'Gak cuma tempat minum kopi. Ini tentang menemukan ruang dimana pikiran lu bisa berkembang.',
    stats: [{ value: '500+', label: 'Buku Pilihan' }, { value: '24', label: 'Workspace' }, { value: '4.9', label: 'Rating' }],
    primaryButtonText: 'Reservasi Sekarang',
    secondaryButtonText: 'Lihat Menu',
    badges: [{ icon: 'star', text: '4.9/5 dari 300+ reviews' }, { icon: 'check', text: 'Reservasi gratis & mudah' }]
  } },
  { type: 'CTASplit', label: 'CTA - Split Layout', iconComponent: PhPackage, defaultProps: { title: 'Subscribe Newsletter', buttonText: 'Subscribe' } },
  { type: 'TestimonialCard', label: 'Testimonial', iconComponent: PhTextbox, defaultProps: { name: 'John Doe', role: 'CEO', quote: 'Outstanding service!' } },
  { type: 'GalleryGrid', label: 'Gallery Grid', iconComponent: PhImage, defaultProps: { title: 'Our Gallery' } },
  { type: 'PricingCard', label: 'Pricing Card', iconComponent: PhPackage, defaultProps: { title: 'Premium', price: '99K', period: 'month' } },
  { type: 'FAQSection', label: 'FAQ', iconComponent: PhTextbox, defaultProps: { title: 'Frequently Asked Questions' } },
  { type: 'TeamGrid', label: 'Team Members', iconComponent: PhSquaresFour, defaultProps: { title: 'Meet The Team' } },
  { type: 'NewsletterForm', label: 'Newsletter Signup', iconComponent: PhPackage, defaultProps: { title: 'Stay Connected', subtitle: 'Get updates' } },
  { type: 'LogoCloud', label: 'Logo/Partner Cloud', iconComponent: PhSquaresFour, defaultProps: { title: 'Partners' } },
  { type: 'VideoEmbed', label: 'Video Player', iconComponent: PhImage, defaultProps: { title: 'Watch Video', videoUrl: '' } },
  { type: 'ContactCard', label: 'Contact Info', iconComponent: PhTextbox, defaultProps: { title: 'Get In Touch' } },
  { type: 'EventsSection', label: 'Events Calendar (API)', iconComponent: PhPackage, defaultProps: { 
  ...
  { type: 'FunFactsSection', label: 'Fun Facts (API)', iconComponent: PhLightning, defaultProps: { 
  ...
  { type: 'LibrarySection', label: 'Book Library Info', iconComponent: PhBook, defaultProps: { 
    badge: 'Library & Workspace',
    title: 'Tempat Otak <span class="text-orange-500">Bekerja</span>',
    description: 'Koleksi buku pilihan, ruang kerja nyaman, dan atmosfer yang bikin lu betah berjam-jam.',
    libraryTitle: 'Perpustakaan Kurated',
    librarySubtitle: '500+ buku pilihan dari berbagai genre',
    libraryFeatures: [
      { title: 'Filosofi & Pemikiran', subtitle: 'Dari Nietzsche sampai Rumi' },
      { title: 'Self-Development', subtitle: 'Upgrade diri lewat bacaan berkualitas' }
    ],
  ...
  { type: 'MerchandiseSection', label: 'Merchandise Store (API)', iconComponent: PhPackage, defaultProps: { 
...
const removeSection = async (index) => {
  const confirmed = await confirm({
    title: 'Hapus Section?',
    message: 'Section ini akan dihapus dari halaman. Tindakan ini tidak dapat dibatalkan.',
    variant: 'danger',
...
const savePage = async (isAuto = false) => {
  ...
  try {
    ...
    console.log(`✅ Page ${isAuto ? 'auto-' : ''}saved successfully at ${lastSaved.value.toLocaleTimeString()}`)
  } catch(e) {
...
const fetchPage = async () => {
  try {
    ...
    if (page.sections && Array.isArray(page.sections)) {
      sections.value = page.sections
        ...
        .map(s => {
          ...
          if (!widgetType) {
            ...
            console.warn(`[FetchPage] componentType missing for section ${s.id}. Fallback to name: ${widgetType}`)
          }
          
          // Remove componentType from props (it's metadata, not a prop)
          const { componentType, ...props } = contentData
          
          console.log(`[FetchPage] ✅ Section ID: ${s.id}, Type: ${widgetType}, Props:`, props)

          // Validate widget exists in registry
          if (!widgetComponents[widgetType]) {
            console.warn(`[FetchPage] ⚠️ Widget type "${widgetType}" not found in registry. Using TextBlock as fallback.`)
          }
      ...
      console.log(`[FetchPage] 🎉 Successfully loaded ${sections.value.length} sections`)
...
onBeforeRouteLeave(async (to, from, next) => {
  ...
  if (saving.value || autoSaving.value) {
    const confirmed = await confirm({
      ...
      message: 'Penyimpanan sedang berlangsung. Yakin ingin keluar? Perubahan mungkin tidak tersimpan.',
      variant: 'warning',
