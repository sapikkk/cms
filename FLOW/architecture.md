...
Versi Dokumen: 2.1 (Enhanced Folder Structure)
...
// 2. Product Intelligence (Coffee Management)
model Product {
  id          String   @id @default(uuid())
  name        String
  slug        String   @unique
...
model Ingredient {
  id        String   @id @default(uuid())
  name      String   // e.g. "Arabica Beans", "Caramel Syrup"
  amount    Decimal  // e.g. 18, 20
  unit      UnitType // e.g. GRAM, ML
  iconUrl   String?  // Custom icon uploaded by Media
  cost      Decimal? // Cost per unit (untuk hitung HPP internal)
...
model ProductVariant {
  id        String  @id @default(uuid())
  name      String  // e.g. "Hot", "Ice", "Large"
  priceAdj  Decimal // Harga tambahan atau harga fix
...
model Book {
  ...
  coverUrl    String
  contentUrl  String? // Link download PDF (jika digital)
  price       Decimal @default(0)
  
  // Fitur Kustomisasi Tampilan Buku (Full Customize oleh Media)
  // Disimpan sebagai JSON agar strukturnya fleksibel
  // Contoh: { "titleFont": "Merriweather", "bgColor": "#F5F5DC", "accentColor": "#8B0000" }
  styleConfig Json?
...
// 4. CMS Core (Dynamic Pages)
model Page {
  id          String  @id @default(uuid())
  title       String
  slug        String  @unique // URL Path (e.g., "about-us", "promo")
  isPublished Boolean @default(false)
  inNavbar    Boolean @default(false)
  navOrder    Int     @default(0)
  
  // INTI CMS: Struktur Halaman
  // Menyimpan array section. Contoh:
  // [
  //   { "type": "Hero", "props": { "title": "Welcome", "img": "..." } },
  //   { "type": "ProductGrid", "props": { "category": "coffee" } }
  // ]
  sections    Json
  
  // SEO Config per halaman
  metaTitle   String?
  metaDesc    String?
...
model ActivityLog {
  id        String   @id @default(uuid())
  action    String   // "CREATE", "UPDATE", "DELETE", "LOCK", "EXPORT"
  entity    String   // "Product", "Page", "User"
  targetId  String?  // ID dari item yang diubah
  details   Json?    // Snapshot data sebelum & sesudah (Old vs New)
...
Fase 5: Security & Audit: Implementasi Middleware Log dan Master Lock.
